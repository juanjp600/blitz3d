...
Vector TerrainRep::getNormal( int x,int z )const{
	Vector 
		vt( (float)x  ,getHeight(x,z)  ,(float)z ),
		v0( (float)x  ,getHeight(x,z-1),(float)z-1 ),
		v1( (float)x+1,getHeight(x+1,z),(float)z ),
		v2( (float)x  ,getHeight(x,z+1),(float)z+1 ),
		v3( (float)x-1,getHeight(x-1,z),(float)z );
	return (
		Plane( vt,v1,v0 ).n+
		Plane( vt,v2,v1 ).n+
		Plane( vt,v3,v2 ).n+
...
TerrainRep::Error TerrainRep::calcErr( int id,const Vert &v0,const Vert &v1,const Vert &v2 )const{
	...
	et.error = 0;
	et.bound = y>=1 ? 255 : (unsigned char)ceil(y*255.0f);

	if( id>=end_tri_id ) return et;

	Vert tv( (v1.x+v2.x)/2,(v1.z+v2.z)/2 );
	float e=fabs(tv.v.y-(v1.v.y+v2.v.y)/2);
	et.error= e>=1 ? 255 : (unsigned char)ceil( (e-EPSILON)*255.0f );
